...
void main()
{
    ...
    if ( object_id == SPHERE )
    {
        // Coordenadas de textura da esfera, computadas com
        ...
        // Seja sc_rho = 1 para simplificar os cálculos

        vec4 bbox_center = (bbox_min + bbox_max) / 2.0;

        vec4 ps = bbox_center + (position_model - bbox_center) / length(position_model - bbox_center);

        float sc_tetha = atan(ps.x, ps.z);
        float sc_phi = asin(ps.y);

        U = (sc_tetha + M_PI) / (2 * M_PI);
        V = (sc_phi + M_PI_2) / M_PI;
    }
    else if ( object_id == BUNNY )
    {
        // Coordenadas de textura do coelho, computadas com
    ...
    if ((object_id == BUNNY) || (object_id == SPHERE)){
        ...
        color.rgb = KdOP * 0.5 + vec3(0.325, 0.745, 0.937) * 0.5; 
